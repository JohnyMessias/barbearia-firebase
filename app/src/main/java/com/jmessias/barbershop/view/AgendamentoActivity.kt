...
import com.jmessias.barbershop.databinding.ActivityAgendamentoBinding
        ...
        binding.btAgendar.setOnClickListener {
            ...
            when {
                ...
                hora < "8:00" && hora > "19:00" -> {
                    mensagem(
                        it,
                        "Barber Shop está fechado, horário de atendimento das 08:00 as 19:00",
                        "#FF0000"
                    )
                }
                ...
                barbeiro1.isChecked && data.isNotEmpty() && hora.isNotEmpty() -> {
                    salvarAgendamento(it, nome, barbeiro1.text.toString(),data, hora)
                }

                barbeiro2.isChecked && data.isNotEmpty() && hora.isNotEmpty() -> {
                    salvarAgendamento(it, nome, barbeiro2.text.toString(),data, hora)
                }

                barbeiro3.isChecked && data.isNotEmpty() && hora.isNotEmpty() -> {
                    salvarAgendamento(it, nome, barbeiro3.text.toString(),data, hora)
                }
